# Identification of unique trip events in CPFV fisheries data (trips unidentified by CDFW)
...
catch_data<-subset(catch_data,is.na(TripType)) #subset to data to multi-day trip type (as deined by CDFW)
...
#     trip_split_new=ifelse(trip_split_date==1&trip_split==0&time_diff==0,1,trip_split_new)) #fix rare event where same vessel is departing at same time as vessel returned 2 days ago
# #trip_split_new=ifelse(n_date_time==1&n_dates==1&day_diff=0&trip_split_new==0,1,trip_split_new))
...
write.csv(catch_summary,file="./FisheriesData/Clean/na_trips_clean.csv",row.names = F)
